      ...
      <div className="h-screen flex flex-col bg-background">
        ...
        <div className="flex-1 overflow-y-auto flex items-start justify-center px-6 py-6">
          ...
          >
            ...
            {/* Action Button — always route leaf photo to AI Remedial Treatment */}
            {(() => {
              const primaryDef = leafResult.deficiencies[0];
              const issueName = primaryDef
                ? primaryDef.nutrient
                : leafResult.overall_health === 'good'
                  ? t('Healthy Plant', 'Tanaman Sihat')
                  : t('General Issue', 'Isu Umum');
              const issueSeverity = primaryDef?.severity || 'moderate';
              const issueEvidence = primaryDef?.visual_evidence || leafResult.recommendation;
