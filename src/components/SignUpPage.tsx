...
import signupFarmBottom from '@/assets/signup-farm-bottom.jpg';
    ...
    <div className="h-screen flex overflow-hidden">
      ...
      <div className="hidden md:flex w-1/2 relative">
        ...
        <div className="relative z-10 flex flex-col justify-end p-10 pb-16">
          <h2 className="font-serif-display text-4xl lg:text-5xl font-extrabold text-white leading-[1.1] tracking-tight" style={{ textShadow: '0 2px 20px rgba(0,0,0,0.3)' }}>
        ...
        <svg className="absolute right-0 top-0 h-full w-16" viewBox="0 0 64 800" fill="none" preserveAspectRatio="none">
          <path d="M64 0H32C32 0 0 100 16 200S64 300 48 400 0 500 16 600S64 700 48 800H64V0Z" fill="hsl(152 20% 98%)" />
        </svg>
      </div>

      {/* Right — Form */}
      <div className="w-full md:w-1/2 flex flex-col relative overflow-y-auto" style={{ background: 'hsl(152 20% 98%)' }}>
        <button onClick={onBack} className="absolute top-4 left-4 md:top-6 md:left-6 flex items-center gap-1.5 text-sm font-body text-muted-foreground hover:text-foreground transition-colors z-10">
          <ArrowLeft size={16} />
          {t(lang, 'Home', 'Utama')}
        </button>
          ...
          <div className="w-full max-w-md">
            ...
            <div className="bg-card rounded-2xl p-6 shadow-lg border border-border/40">
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    <button
                      onClick={() => setLang('bm')}
                      className={`flex-1 rounded-xl py-2.5 font-body text-sm font-semibold transition-all duration-200 active:scale-[0.97] ${lang === 'bm' ? 'bg-primary text-primary-foreground shadow-md' : 'bg-muted border border-border text-muted-foreground hover:border-primary/40'}`}
                    >
                      Bahasa Malaysia
                    </button>
                    <button
                      onClick={() => setLang('en')}
                      className={`flex-1 rounded-xl py-2.5 font-body text-sm font-semibold transition-all duration-200 active:scale-[0.97] ${lang === 'en' ? 'bg-primary text-primary-foreground shadow-md' : 'bg-muted border border-border text-muted-foreground hover:border-primary/40'}`}
                    >
                      English
                    </button>
                  </div>
                ...
                <div className="relative">
                  <input
                    ...
                    placeholder=" "
                    className="peer w-full rounded-xl border border-border bg-muted/50 px-4 pt-5 pb-1.5 text-sm text-foreground focus:outline-none focus:border-primary focus:ring-2 focus:ring-primary/30 transition-all"
                  />
              ...
              <button
                ...
                className="w-full mt-6 rounded-xl py-3 font-body font-extrabold text-sm flex items-center justify-center gap-2 btn-gradient-primary"
              >
        ...
        {/* Bottom Farm Image Section — layered */}
        <div className="relative h-48 md:h-56 overflow-hidden flex-shrink-0">
          <img
            src={signupFarmBottom}
            alt="Thriving Malaysian farmland"
            className="absolute inset-0 w-full h-full object-cover"
            loading="lazy"
            width={1280}
            height={512}
          />
          <div className="absolute inset-0 bg-gradient-to-b from-[hsl(152_20%_98%)] via-transparent to-transparent" />
