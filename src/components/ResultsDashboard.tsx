      ...
      <Tabs defaultValue="summary" className="flex-1 flex flex-col overflow-hidden print:hidden">
        ...
        <div className="flex-1 overflow-y-auto px-4 md:px-8 py-5">
          ...
          <TabsContent value="summary" className="mt-0 space-y-5">
            ...
            {/* Nutrient Deficit Section — Leaf Photo: show deficiency cards; otherwise: radar chart */}
            {result.input_mode === 'leaf_photo' && leafDeficiencies && leafDeficiencies.length > 0 ? (
              <motion.div
              ...
              >
                ...
                <div className="flex items-center gap-2 mb-4 relative z-10">
                  ...
                  <h3 className="font-display text-lg font-bold text-emerald-50 tracking-tight">
                    {t(lang, 'Deficiencies Detected', 'Kekurangan Dikesan')}
                  </h3>
                </div>
                <div className="space-y-3 relative z-10">
                  {leafDeficiencies.map((d, i) => {
                    const shortEvidence = d.visual_evidence.length > 100
                      ? (d.visual_evidence.slice(0, d.visual_evidence.indexOf('.', 30) + 1) || d.visual_evidence.slice(0, 100) + '…')
                      : d.visual_evidence;
                    const sevColor = d.severity === 'severe' ? 'bg-red-500/20 text-red-300 border-red-400/30'
                      : d.severity === 'moderate' ? 'bg-amber-500/20 text-amber-300 border-amber-400/30'
                      : 'bg-emerald-500/20 text-emerald-300 border-emerald-400/30';
                    return (
                      <div key={i} className="rounded-xl p-4 bg-white/5 border border-white/10">
                        <div className="flex items-center justify-between mb-2">
                          <span className="text-base font-sans font-bold text-emerald-50 capitalize">{d.nutrient}</span>
                          <span className={`px-3 py-1 rounded-full text-xs font-sans font-bold capitalize border ${sevColor}`}>
                            {d.severity} · {d.estimated_deficit_pct}%
                          </span>
                        </div>
                        <p className="text-sm text-emerald-200/70 font-sans leading-snug">{shortEvidence}</p>
                      </div>
                    );
                  })}
                </div>
