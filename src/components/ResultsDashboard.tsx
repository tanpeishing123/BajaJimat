...
export function ResultsDashboard({ lang, result, cropType, onBack, backLabel, onToggleLang }: Props) {
  ...
  const buildWhatsAppText = () => {
    const lines = [
      `🌿 BajaJimat ${t(lang, 'Fertilizer Prescription', 'Preskripsi Baja')}`,
      '',
      ...fertItems.map(r => `• ${r.name}: ${r.bags} ${t(lang, 'bags', 'beg')} × RM${r.price_per_bag} = RM${r.subtotal_rm}`),
      ...mgItems.map(r => `• ${r.name}: ${r.bags} ${t(lang, 'bags', 'beg')} × RM${r.price_per_bag} = RM${r.subtotal_rm}`),
      '',
      `${t(lang, 'Total', 'Jumlah')}: RM${displayTotalCost.toFixed(2)}`,
      `💰 ${t(lang, 'Saved', 'Jimat')}: RM${result.savings_rm}`,
    ];
