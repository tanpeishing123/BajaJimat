            ...
            >
              ...
              <div className="flex justify-center gap-1.5 sm:gap-4 mt-2 sm:mt-3 relative z-10 flex-wrap">
                {[
                  ...
                  <div key={d.nutrient} className={`flex items-center gap-1 sm:gap-2.5 px-2 sm:px-6 py-1.5 sm:py-3 rounded-full ${d.pill} border ${d.border} min-w-0`}>
                    <span className={`text-[10px] sm:text-sm font-extrabold font-display ${d.text} tracking-wide shrink-0`}>{d.nutrient}</span>
                    <span className="text-xs sm:text-lg font-bold text-white font-sans tabular-nums truncate">{d.value}kg</span>
                  </div>
                ))}
              </div>
            </motion.div>
