              ...
              <div className="h-[150px] sm:h-[210px] relative z-10">
                <ResponsiveContainer width="100%" height="100%">
                  <RadarChart cx="50%" cy="50%" outerRadius="68%" data={radarData}>
                    ...
                    <PolarAngleAxis dataKey="nutrient" tick={({ x, y, payload }) => (
                      <text x={x} y={y} fill="rgba(255,255,255,0.85)" fontSize={window.innerWidth < 640 ? 11 : 16} fontWeight={800} fontFamily="Inter, sans-serif" textAnchor="middle" dominantBaseline="central">
                        {payload.value}
                      </text>
                    )} />
              ...
              <div className="flex justify-center gap-1.5 sm:gap-4 mt-2 sm:mt-3 relative z-10 flex-wrap">
                {[
                  ...
                  <div key={d.nutrient} className={`flex items-center gap-1 sm:gap-2.5 px-2.5 sm:px-6 py-1.5 sm:py-3 rounded-full ${d.pill} border ${d.border}`}>
                    ...
                    <span className="text-xs sm:text-lg font-bold text-white font-sans tabular-nums">{d.value}kg</span>
                  </div>
                ))}
              </div>
            </motion.div>
